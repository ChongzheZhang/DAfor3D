...
class SECONDHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if not self.training:
            if self.model_cfg.get("CIA_SSD", False):
                rcnn_iou = torch.clamp((rcnn_iou+1)*0.5, min=0.0)
                cls_pred = torch.sigmoid(targets_dict['roi_scores'].view(-1,1)) * torch.pow(rcnn_iou, 4)
                batch_dict['batch_cls_preds'] = cls_pred.view(batch_dict['batch_size'], -1, rcnn_iou.shape[-1])
            else:
    ...
    def get_box_iou_layer_loss(self, forward_ret_dict):
        ...
        if self.model_cfg.get("CIA_SSD", False):
            rcnn_iou_labels = forward_ret_dict['gt_iou_of_rois'].view(-1).detach()
            rcnn_iou_labels = 2 * rcnn_iou_labels - 1
        else:
        ...
        if self.model_cfg.get("CIA_SSD", False):
            iou_valid_mask = (rcnn_iou_labels >= -1).float()
        else:
