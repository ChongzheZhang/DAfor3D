...
class KittiDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        if self.dataset_cfg.get('RANDOM_DOWNSAMPLE', False) and self.training:
            ...
            i = random.random()
            if i <= rate_64:
                lidar_file = lidar_file / 'velodyne' / ('%s.bin' % idx)
            elif (i > rate_64) & (i <= rate_32):
                lidar_file = lidar_file / 'velodyne_32' / ('%s.bin' % idx)
            else:
                lidar_file = lidar_file / 'velodyne_16' / ('%s.bin' % idx)
        else:
            if self.dataset_cfg.get("USE_LOW_BEAMS", False):
                velodyne = 'velodyne_' + str(self.dataset_cfg.USE_LOW_BEAMS)
                lidar_file = self.root_split_path / velodyne / ('%s.bin' % idx)
            else:
                lidar_file = self.root_split_path / 'velodyne' / ('%s.bin' % idx)
        # print(f"using kitti_dataset.py, path = {lidar_file}")
