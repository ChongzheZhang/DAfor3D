...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.use_resized_label = self.dataset_cfg.get('USE_RESIZED_LABEL', False)
        self.load_kirk = self.dataset_cfg.get('LOAD_KIRK', False)
        if self.load_kirk:
            kirk_split_dir = self.root_path / 'ImageSets' / ('kirk_%s.txt' % self.split)
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            if self.use_resized_label:
                info_path = self.data_path / sequence_name / ('%s_resized_gt_boxes.pkl' % sequence_name)
            else:
    ...
    def set_box_weights(self, infos):
        for info in infos:
    ...
    def resize_gt_box(self):
        self.logger.info('Start resize waymo bounding boxes')
        def rotate_center_point(points, a):
            x = points[0]
            y = points[1]
            new_x = x * np.cos(a) - y * np.sin(a)
            new_y = x * np.sin(a) + y * np.cos(a)
            points[0] = new_x
            points[1] = new_y
            return points
        ...
        num_skipped_infos = 0
        for idx in range(len(self.sample_sequence_list)):
            sequence_name = os.path.splitext(self.sample_sequence_list[idx])[0]
            info_path = self.data_path / sequence_name / ('%s.pkl' % sequence_name)
            f = open(info_path, 'rb')
            infos = pickle.load(f)
            ...
            for k in range(0, len(infos)):
                ...
                for i in range(num_obj):
                    gt_points = points[box_idxs_of_pts == i][:, :3]
                    if type(gt_points) is np.ndarray:
                        if gt_points.size >= 6:
                            gt_points[:, :3] -= gt_boxes[i, :3]
                            gt_points = np.stack([gt_points, gt_points], 0)
                            heading_angle = torch.from_numpy(np.array([gt_boxes[i, 6], gt_boxes[i, 6]]))
                            gt_of_rois = common_utils.rotate_points_along_z(points=gt_points, angle=-heading_angle)
                            gt_of_rois = np.squeeze(gt_of_rois[0])
                            max_in_gt = gt_of_rois.max(axis=0)
                            min_in_gt = gt_of_rois.min(axis=0)
                            new_gt_size = max_in_gt - min_in_gt
                            center_shift = (max_in_gt + min_in_gt) / 2
                            center_shift = rotate_center_point(center_shift, gt_boxes[i, 6])
                            gt_boxes[i, 3:6] = new_gt_size
                            gt_boxes[i, 0:3] += center_shift
                        # else:
                        #     gt_boxes[i, 0:3] += 0.01
                    else:
                        continue

                infos[k]['annos']['gt_boxes_lidar'] = gt_boxes
                infos[k]['annos']['location'] = gt_boxes[:, :3]
                infos[k]['annos']['dimensions'] = gt_boxes[:, 3:6]
                infos[k]['resized'] = True

            new_info_path = self.data_path / sequence_name / ('%s_resized_gt_boxes.pkl' % sequence_name)
            with open(new_info_path, 'wb') as f:
                pickle.dump(infos, f)
            self.logger.info(f'finish the {idx} {self.split} sequence')
...
if __name__ == '__main__':
    ...
    if args.func == 'create_waymo_infos':
        ...
        ROOT_DIR = Path('/data/private/m142_datasets/Waymo/waymo/waymo_format/')
