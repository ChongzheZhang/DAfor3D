...
class DataAugmentor(object):
    ...
    def random_down_sample(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_down_sample, config=config)
        points = data_dict['points']
        assert 0 <= config['GENERAL_RATE'] < 1 and 0 <= config['DENSE_RATE'] < 1
        rate1 = config['GENERAL_RATE']
        rate2 = config['DENSE_RATE']
        downsample_rate = np.ones((points.shape[0],1))*rate1
        rate_mask = (-0.1 < points[:, 2]) & (points[:, 2] < 0.1)
        downsample_rate = downsample_rate + rate_mask[:, None]*(rate2 - rate1)
        random_idx = np.random.rand(points.shape[0],1)
        mask = np.squeeze(random_idx > downsample_rate, -1)
        downsampled_points = points[mask]
        data_dict['points'] = downsampled_points
