    ...
    if swap_pyramid_mask.sum() > 0:
        ...
        if selected_pyramids.sum() > 0:
            ...
            for i in range(num_swapped_pyramids):
                ...
                to_swap_points_intensity_ratio = (to_swap_points[:, 3:] - to_swap_points[:, 3:].min(axis=0)) / \
                                                 np.clip(
                                                     (to_swap_points[:, 3:].max(axis=0) - to_swap_points[:, 3:].min(axis=0)),
                                                     1e-6, 1)
                swapped_points_intensity_ratio = (swapped_points[:, 3:] - swapped_points[:, 3:].min(axis=0)) / \
                                                 np.clip(
                                                     (swapped_points[:, 3:].max(axis=0) - swapped_points[:, 3:].min(axis=0)),
                                                     1e-6, 1)
                ...
                new_to_swap_points_intensity = recover_points_intensity_by_ratio(
                    swapped_points_intensity_ratio, to_swap_points[:, 3:].max(axis=0),
                    to_swap_points[:, 3:].min(axis=0))
                new_swapped_points_intensity = recover_points_intensity_by_ratio(
                    to_swap_points_intensity_ratio, swapped_points[:, 3:].max(axis=0),
                    swapped_points[:, 3:].min(axis=0))
